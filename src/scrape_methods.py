...
def linkedin_newpostings(
    ...
    signinbutton = driver.find_element(
        By.CSS_SELECTOR, "a.nav__button-secondary")

    isLoggedIn = signinbutton is None
    ...
    if (signinbutton):
        ...
        searchbutton = driver.find_element(
            By.CSS_SELECTOR, "button.base-search-bar__submit-btn:nth-child(5)")

        # check to make sure the things grabbed match expectation..
        print(searchlocbox.text)
        assert (searchbutton.find_elements(
            By.CLASS_NAME, "base-search-bar__search-icon"))
    else:
        searchbox = driver.find_element(
            By.CLASS_NAME, "jobs-search-box__text-input")
        searchlocbox = driver.find_element(
            By.ID, "jobs-search-box-location-id-ember31")
        searchbutton = driver.find_element(
    ...
    jobslist = driver.find_element(
        By.CSS_SELECTOR, ".jobs-search__results-list")
    if (isLoggedIn):
        jobslist = driver.find_element(
            By.CLASS_NAME, "job-search-results__list")
